...
def generate_completions(model, tokenizer, prompts, batch_size=1, stop_id_sequences=None, add_special_tokens=True, disable_tqdm=False, **generation_kwargs):
    ...
    for i in range(0, len(prompts), batch_size):
        ...
        tokenized_prompts = tokenizer(batch_prompts, padding="longest", return_tensors="pt", add_special_tokens=add_special_tokens, max_length=tokenizer.model_max_length)
        ...
        try:
            ...
            # batch_outputs = model.generate(input_ids=batch_input_ids,attention_mask=attention_mask,eos_token_id=tokenizer.eos_token_id,stopping_criteria=[KeyWordsCriteria(stop_id_sequences)] if stop_id_sequences else None,**generation_kwargs)
...
def score_completions(model, tokenizer, scoring_examples, batch_size=1, aggregation="sum", disable_tqdm=False):
    ...
    for i in range(0, len(unrolled_examples), batch_size):
        ...
        # ]
        batch_examples = [
            (example["prompt"][:-1] if example["prompt"][-1] in ["\n", " "] else example["prompt"])
            + example["completion"] for example in unrolled_examples[i:i+batch_size]
        ]
