...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=int,
        default=3600,
        help='Timeout for the training process. Useful if tokenization process is long. Default is 3600 seconds (60 minutes).',
    )
    parser.add_argument(
        '--trust_remote_code',
        action='store_true',
...
def encode_with_prompt_completion_format(example, tokenizer, max_seq_length, add_bos=False):
    ...
    if example['prompt'].endswith((' ', '\n', '\t')):
        tokenized_prompt = tokenizer(example['prompt'][:-1], return_tensors='pt', max_length=max_seq_length, truncation=True)
    else:
...
def main():
    ...
    if args.use_lora:
        ...
        if 'INCITE' in args.model_name_or_path:
            ...
            peft_config = LoraConfig(
                ...
                lora_alpha=args.lora_alpha, 
                lora_dropout=args.lora_dropout,
                target_modules=["blocks.0.attn_out"]
